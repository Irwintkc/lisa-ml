...
def main(config_path):
    ...
    logger.info(
        "Data loaded. Input dimension: %d, Number of model classes: %d",
        input_dim,
        num_model_classes,
    )
    ...
    if config.get("test", False):
        ...
        if model_choice == "NeuralForest":
            ...
            training_results = train_ndp(
                ...
                lr=lr,
                report_every=config["training_params"].get("report_every", 10),
                n_class=2,
                jointly_training=config.get("jointly_training", False),
                logger=logger.info,
            )
        else:
            training_results = train_model(
                ...
                train_loader,
                val_loader,
                focal_loss_config=focal_loss_config,
                num_epochs=num_epochs,
                lr=lr,
                logger=logger.info,
    ...
    # Compute saliency.
    avg_saliency, params_impact, params_std = compute_saliency(
        model_instance, val_loader.dataset, num_samples=10
    )
    ...
    for feature, sal, pimp, pstd in zip(
        selected_features, avg_saliency_list, params_impact_list, params_std_list
    ):
        logger.info(
            "Feature: %s, Average Saliency: %.4f, Parameters Impact: %.4f, Parameters Std: %.4f",
            feature,
            sal,
            pimp,
            pstd,
    ...
    if  model_choice == 'Multi_L':
        ...
        model_true_overall_label = np.array(
            ["Model " + label.split()[1].split(".")[0] for label in model_true_labels]
        )
        overall_class_names = ["Model 1", "Model 2", "Model 3"]
        cm_overall = confusion_matrix(
            model_true_overall_label, overall_best_model, labels=overall_class_names
        )
        plot_confusion_matrix_multiclass(
            cm_overall,
            class_names=overall_class_names,
            save_dir=fig_dir,
            filename="cm_model_overall.png",
        )
    ...
    X_val_df["Predicted_Class_binary"] = (
        X_val_df["Predicted_Class"] > evalutaion_threshold
    ).astype(int)
    ...
    X_val_df["Prediction_Status"] = np.where(
        X_val_df["Predicted_Class_binary"] == X_val_df["True_Class"], "Correct", "Wrong"
    )
    ...
    plot_corner(
        df_class1,
        selected_features,
        hue_col="Prediction_Status",
        save_path=class1_status_plot_path,
    ...
    plot_corner(
        ...
        selected_features,
        hue_col="Prediction_Status",
        save_path=class0_status_plot_path,
        palette=prediction_status_palette,
    )
