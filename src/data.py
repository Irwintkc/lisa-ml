...
def load_data(config):
    ...
    (
        ...
        y_binary_train_val,
        y_model_train_val,
        default_weights.loc[X_train_val.index],
        test_size=0.2,
        random_state=val_seed_val,
    )
    ...
    if n_samples_class0 and n_samples_class1 is not None:
        ...
        sample_df = generate_sample_dataframe(
            train_df, loaded_fim_data, n_samples_class0, n_samples_class1
        )
    ...
    test_set_df = pd.DataFrame(X_test_scaled, columns=X_test.columns)
    ...
    test_set_save_path = os.path.join(
        "data", "test_set", f"test_set_scaled_seed_{test_seed_val}_{name_of_run}.csv"
    )
    ...
    test_set_original_df = pd.DataFrame(X_test_original, columns=X_test.columns)
    ...
    test_set_original_save_path = os.path.join(
        "data", "test_set", f"test_set_original_seed_{test_seed_val}_{name_of_run}.csv"
    )
